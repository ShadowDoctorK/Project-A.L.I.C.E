...
{
    public class Monitor_Json : MonitorBase
    {
        #region Properties
        //Settings
        ...
        private void UpdateStatus()
        {
            ...
            try
            {
                ...
                if (Value.Flags >= 131072)
                {
                    ...
                    IStatus.FrameShiftDrive.U_Charging(true);                    
                }
                else
                {
                    IStatus.FrameShiftDrive.U_Charging(false);
                }
                #endregion
                ...
                if (Value.Flags >= 2048)
                {
                    ...
                    if (IStatus.Fuel.ScoopingCommenced == false && ICheck.InitializedJson(MethodName))
                    {
                        IStatus.Fuel.Scooping(MethodName);
                    }
                }
                else
                {
                    ...
                    if (IStatus.Fuel.ScoopingCommenced == true && ICheck.InitializedJson(MethodName))
                    {
                        IStatus.Fuel.Scooping(MethodName);
                    }
                    
                    //Reset variables once we finish scooping.
                    if (IStatus.Fuel.ScoopingCompleted == true && ICheck.InitializedJson(MethodName))
                    {
                        Thread.Sleep(100);
                ...
                {                   
                    ...
                    IStatus.Shields.Update(true);
                }
                //Shields Offline...
                else
                {
                    IStatus.Shields.Update(false);
                }
                #endregion
                ...
                IActions.Hardpoints.Current = Value.FireGroup + 1;
                #endregion
        ...
        public class FileCargo : FileBase
        {
            public FileCargo(bool E)
            {
                ...
                Enabled = E;
                Name = "Cargo.json";
                Dir = new DirectoryInfo(Paths.LogDirectory);
            }
        }
