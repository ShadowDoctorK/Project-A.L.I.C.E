...
namespace ALICE_Response
{
    ...
    {
        public static Pulse_Wave_Analyser PulseWaveAnalyser = new Pulse_Wave_Analyser();
    }

    public class Pulse_Wave_Analyser : Generic_Equipment
    {
        /// <summary>
        ...
        {
            if (PlugIn.MasterAudio == false) { Logger.Log(ClassName, "Pulse Wave Analyser Assigned To Group " + Group + " " + FireMode + " Fire.", Logger.Yellow); }

            Speech.Speak(""
                .Phrase(GN_Positive.Default, true)
                .Phrase("Pulse Wave Analyser Assigned To Group [GROUP], [FIREMODE] Fire.")
                .Token("[GROUP]", Group)
        ...
        {
            if (PlugIn.MasterAudio == false) { Logger.Log(ClassName, "Pulse Wave Analyser Not Installed.", Logger.Yellow); }

            Speech.Speak(""
                .Phrase(GN_Negative.Default, true)
                .Phrase("Pulse Wave Analyser Not Installed."),
                CA, V1, V2, V3, P, V);
        }
    }
}