...
namespace ALICE_Response
{
    ...
    {
        public static Kill_Warrent_Scanner KillWarrentScanner = new Kill_Warrent_Scanner();
    }

    public class Kill_Warrent_Scanner : Generic_Equipment
    {
        /// <summary>
        ...
        {
            if (PlugIn.MasterAudio == false) { Logger.Log(ClassName, "Kill Warrent Scanner Assigned To Group " + Group + " " + FireMode + " Fire.", Logger.Yellow); }

            Speech.Speak(""
                .Phrase(GN_Positive.Default, true)
                .Phrase("Kill Warrent Scanner Assigned To Group [GROUP], [FIREMODE] Fire.")
                .Token("[GROUP]", Group)
        ...
        {
            if (PlugIn.MasterAudio == false) { Logger.Log(ClassName, "Kill Warrent Scanner Not Installed.", Logger.Yellow); }

            Speech.Speak(""
                .Phrase(GN_Negative.Default, true)
                .Phrase("Kill Warrent Scanner Not Installed."),
                CA, V1, V2, V3, P, V);
        }
    }
}