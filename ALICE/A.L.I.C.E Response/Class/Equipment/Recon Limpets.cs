...
namespace ALICE_Response
{
    ...
    {
        public static Recon_Limpets ReconLimpets = new Recon_Limpets();
    }

    public class Recon_Limpets : Generic_Equipment
    {
        /// <summary>
        ...
        {
            if (PlugIn.MasterAudio == false) { Logger.Log(ClassName, "Recon Limpet Controller Assigned To Group " + Group + " " + FireMode + " Fire.", Logger.Yellow); }

            Speech.Speak(""
                .Phrase(GN_Positive.Default, true)
                .Phrase("Recon Limpet Controller Assigned To Group [GROUP], [FIREMODE] Fire.")
                .Token("[GROUP]", Group)
        ...
        {
            if (PlugIn.MasterAudio == false) { Logger.Log(ClassName, "Recon Limpet Controller Not Installed.", Logger.Yellow); }

            Speech.Speak(""
                .Phrase(GN_Negative.Default, true)
                .Phrase("Recon Limpet Controller Not Installed."),
                CA, V1, V2, V3, P, V);
        }
    }
}