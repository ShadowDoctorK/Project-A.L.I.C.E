...
namespace ALICE_Keybinds
{
	public class AliceKeys
	{
		...
		public Dictionary<string, int> VirtualKeys = new Dictionary<string, int>()
		{
			{ "Key_Backspace", 8 },
			{ "Key_Tab", 9 },
			{ "Key_Enter", 13 },
		...
		public void Press(string Key, int ExtraSleep = 0, string Delay = null, int Duration = 50)
		{
			...
			try
			{ IPlatform.ExecuteCommand("(A.L.I.C.E) Keybind: " + Key, true); return; }
			catch (Exception ex)
			{ Logger.Exception(MethodName, "Exception: " + ex); return; }
			finally
			{
		...
		public void Load_VoiceAttackVariables()
		{
			...
			foreach (KeyValuePair<string, Bind> bind in Keybinds)
			{
				...
				if (bind.Value.Key1 != "" && bind.Value.Key1 != null)
				{
					...
					string Variable = "[" + VirtualKeys[bind.Value.Key1] + "]";
					if (bind.Value.Key2 != null && bind.Value.Key2 != "") { Variable = Variable + "[" + VirtualKeys[bind.Value.Key2] + "]"; }
					if (bind.Value.Key3 != null && bind.Value.Key3 != "") { Variable = Variable + "[" + VirtualKeys[bind.Value.Key3] + "]"; }
					if (bind.Value.Key4 != null && bind.Value.Key4 != "") { Variable = Variable + "[" + VirtualKeys[bind.Value.Key4] + "]"; }
